...
def run_multihead_self_attention(
    ...
    # causal mask，矩阵下三角
    # 确保在计算注意力时，每个位置只能关注之前的位置，不能关注之后的位置
    seq_len = in_features.shape[-2]
    mask = torch.tril(torch.ones(seq_len, seq_len, device=mha.device)).bool()
    
    return mha.forward(in_features, mask=mask)
...
def run_multihead_self_attention_with_rope(
    ...
    return mha.forward(in_features, mask=mask, token_positions=token_positions)
...
def run_transformer_lm(
    ...
    """
    from cs336_basics.transformer.lm import Transformer_LM
...
def run_silu(in_features: Float[Tensor, " ..."]) -> Float[Tensor, " ..."]:
    ...
    """
    from cs336_basics.transformer.ffn import silu
...
def run_get_lr_cosine_schedule(
    ...
    """
    from cs336_basics.training.schedule import lr_scheduler
    return lr_scheduler(it, max_learning_rate, min_learning_rate, warmup_iters, cosine_cycle_iters)
